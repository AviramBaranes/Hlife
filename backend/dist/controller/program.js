...
exports.changeProgram = exports.getProgram = exports.getAllPrograms = exports.createProgram = void 0;
...
const createProgram = async (req, res, next) => {
    try {
        ...
        let workout;
        if (trainingDayName) {
            workout = await Workout_1.default.findOne({ trainingDayName, user: userId });
        }
...
const changeProgram = async (req, res, next) => {
    try {
        ...
        if (Object.keys(requestedProgram).length === 0) {
            res
                .status(401)
                .send("No program has made for this user. Make sure you create one first");
            return;
        }
        const isProgramAlreadySet = requestedProgram.restDay || requestedProgram.workout;
        if (!isProgramAlreadySet) {
        ...
        let workout;
        if (trainingDayName) {
            workout = await Workout_1.default.findOne({ user: userId, trainingDayName });
        }
        if (!workout && trainingDayName) {
            res
                .status(401)

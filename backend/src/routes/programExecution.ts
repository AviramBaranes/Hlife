...
router.post(
  ...
  [
    param('date', 'The parameter that provided is not a valid date')
      .optional()
      .isDate({ format: 'MM-DD-YYYY' }),
    body('exercises', 'Each exercise need to have a boolean value')
      .optional()
      .custom((value: { [name: string]: string }) => {
...
router.get(
  '/:date',
  authMiddleware,
  [param('date', 'This date is invalid').isDate({ format: 'MM-DD-YYYY' })],
...
router.get(
  ...
  [
    ...
    param('date', 'date is invalid').isDate({ format: 'MM-DD-YYYY' }),
  ],

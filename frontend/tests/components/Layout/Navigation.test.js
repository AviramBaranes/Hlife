...
const middllewares = getDefaultMiddleware();
const mockStore = configureStore(middllewares);

describe("Navigation", () => {
  test("should render the dom correctly (test by elements)", () => {
    const initalState = { usersReducer: { isAuthenticated: false } };
  ...
  test("should render the dom correctly (test by text)", () => {
    const initalState = { usersReducer: { isAuthenticated: true } };

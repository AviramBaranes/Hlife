...
describe("test payload", () => {
  test("should call axios with the right payload (forgot-password) ", async () => {
    ...
    await waitFor(() => {
      expect(window.axiosPayloadTests.sendEmailSubmitPayload).toStrictEqual(
        expectedParamsFromAxios
      );
    });
  });
  test("should call axios with the right payload (login) ", async () => {
    ...
    await waitFor(() => {
      expect(window.axiosPayloadTests.loginSubmitPayload).toStrictEqual(
        expectedParamsFromAxios
      );
    });
  });
  test("should call axios with the right payload (signup) ", async () => {
    ...
    await waitFor(() => {
      expect(window.axiosPayloadTests.signUpSubmitPayload).toStrictEqual(
        expectedParamsFromAxios
      );
    });
  });
